;; reblocks like framework based on htmlx

(require :hunchentoot)
(require :html-actions)
(require :spinneret)

(defpackage :htmlx-demo-4
  (:use :cl))

(in-package :htmlx-demo-4)

(defun gen-widget-id ()
  (string-downcase (princ-to-string (gensym "W"))))

(defclass widget ()
  ((id :accessor widget-id
       :initform (gen-widget-id))))

(defgeneric render (widget))
(defgeneric update (widget))

(defmethod update (widget))
